...
## Additive                           ##
########################################
icar.ad.fe.fe <- inla(OBS ~ -1 + I1 + I2 +
...
## Type I                             ##
########################################
icar.t1.fe.fe <- inla(OBS ~ -1 + I1 + I2 +
                        f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                        f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k))) +
...
## Type II                            ##
########################################
icar.t2.fe.fe <- inla(OBS ~ -1 + I1 + I2 +
                        f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) +
                        f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## Type III                           ##
########################################
icar.t3.fe.fe<- inla(OBS ~ -1 + I1 + I2 +
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k)) ) +
                       f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## Type IV                            ##
########################################
icar.t4.fe.fe <- inla(OBS ~ -1 + I1 + I2 +
                        f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                        f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
