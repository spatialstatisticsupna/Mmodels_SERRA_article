...
model.s <- inla.rgeneric.define(inla.rgeneric.Mmodel.model.icar.n, debug = TRUE, k = k, W = W, alpha.min = alpha.min, alpha.max = alpha.max)
...
## Additive                           ##
########################################
icar.ad.re.re<- inla(OBS ~ -1 + I1 + I2 + 
...
## Type I                             ##
########################################
icar.t1.re.re<- inla(OBS ~ -1 + I1 + I2 +
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                       f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k))) +
...
## Type II                            ##
########################################
icar.t2.re.re <- inla(OBS ~ -1 + I1 + I2 +
                        f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) +
                        f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## Type III                           ##
########################################
icar.t3.re.re<- inla(OBS ~ -1 + I1 + I2 +
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k)) ) + 
                       f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## Type IV                            ##
########################################
icar.t4.re.re<- inla(OBS ~ -1 + I1 + I2 +
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                       f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## save                               ##
########################################
resulta.inla.icar.re<- list(icar.ad.re=icar.ad.re.re,
                            icar.t1.re=icar.t1.re.re,
                            icar.t2.re=icar.t2.re.re,
                            icar.t3.re=icar.t3.re.re,
                            icar.t4.re=icar.t4.re.re)
save(resulta.inla.icar.re, file=paste0(gsub("\\.", "_", "resulta.inla.icar.re"), ".RData" ) )
