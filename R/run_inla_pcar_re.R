...
## load functions                                                             ##
################################################################################
source("functions_INLA/inla_rgeneric_Mmodel_model_pcar_normal.R") # pcar
model.s <- inla.rgeneric.define(inla.rgeneric.Mmodel.model.pcar.n, debug = TRUE, k = k, W = W, alpha.min = alpha.min, alpha.max = alpha.max)
...
## Additive                           ##
########################################
pcar.ad.re.re<- inla(OBS ~ -1 + I1 + I2 + 
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
...
## Type I                             ##
########################################
pcar.t1.re.re<- inla(OBS ~ -1 + I1 + I2 +
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                       f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k))) +
...
## Type II                            ##
########################################
pcar.t2.re.re<- inla(OBS ~ -1 + I1 + I2 +
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                       f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## Type III                           ##
########################################
pcar.t3.re.re<- inla(OBS ~ -1 + I1 + I2 +
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                       f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## Type IV                            ##
########################################
pcar.t4.re.re<- inla(OBS ~ -1 + I1 + I2 +
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                       f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## save                               ##
########################################
resulta.inla.pcar.re<- list(pcar.ad.re=pcar.ad.re.re,
                            pcar.t1.re=pcar.t1.re.re,
                            pcar.t2.re=pcar.t2.re.re,
                            pcar.t3.re=pcar.t3.re.re,
                            pcar.t4.re=pcar.t4.re.re)
save(resulta.inla.pcar.re, file=paste0(gsub("\\.", "_", "resulta.inla.pcar.re"), ".RData" ) )
