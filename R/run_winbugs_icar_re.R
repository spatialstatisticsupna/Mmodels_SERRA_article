...
## load functions                                                             ##
################################################################################
source("functions_WinBUGS/nva_nva_icar_spaciotemporal_functions.R") # functions (models)
...
param<-c("smr.prob", "espat.prob", "etemp.prob", "SMR", "lambda", "mu", "Theta", "M", "sdstruct", "prec",  "Sigma.s", "Corre.s", "Gam", "Mg", "sdstructg", "precg", "Sigma.t", "Corre.t", "Espat", "Etemp")
t.result <- system.time(result <- Pbugs(program="winbugs", data = data, inits = initials, parameters.to.save = param, model.file = NVA.NVA.ad.icar,
                                        n.chains = num.chains, n.iter = num.iter, n.burnin = num.burnin, bugs.directory = bugs.dir, DIC = FALSE))
resulta.nva.nva.icar.ad<- result 
t.resulta.nva.nva.icar.ad<- t.result
## rm
...
param<-c("smr.prob", "eint.prob", "espat.prob", "etemp.prob", "SMR", "lambda", "mu", "Theta", "M", "sdstruct", "prec",  "Sigma.s", "Corre.s", "Gam", "Mg", "sdstructg", "precg", "Sigma.t", "Corre.t", "Espat", "Etemp", "Eint", "Zet", "sdZet")
t.result <- system.time(result <- Pbugs(program="winbugs", data = data, inits = initials, parameters.to.save = param, model.file = NVA.NVA.t1.icar,
                                        n.chains = num.chains, n.iter = num.iter, n.burnin = num.burnin, bugs.directory = bugs.dir, DIC = FALSE))
resulta.nva.nva.icar.t1<- result 
t.resulta.nva.nva.icar.t1<- t.result
## rm
rm(list = c("cl", "initials", "param", "result", "t.result"))
rm("data")
...
param<-c("smr.prob", "eint.prob", "espat.prob", "etemp.prob", "SMR", "lambda", "mu", "Theta", "M", "sdstruct", "prec",  "Sigma.s", "Corre.s", "Gam", "Mg", "sdstructg", "precg", "Sigma.t", "Corre.t", "Espat", "Etemp", "Eint", "Zet", "sdZet")
t.result <- system.time(result <- Pbugs(program="winbugs", data = data, inits = initials, parameters.to.save = param, model.file = NVA.NVA.t2.icar,
                                        n.chains = num.chains, n.iter = num.iter, n.burnin = num.burnin, bugs.directory = bugs.dir, DIC = FALSE))
resulta.nva.nva.icar.t2<- result 
t.resulta.nva.nva.icar.t2<- t.result
## rm
rm(list = c("cl", "initials", "param", "result", "t.result"))
rm("data")
...
param<-c("smr.prob", "eint.prob", "espat.prob", "etemp.prob", "SMR", "lambda", "mu", "Theta", "M", "sdstruct", "prec",  "Sigma.s", "Corre.s", "Gam", "Mg", "sdstructg", "precg", "Sigma.t", "Corre.t", "Espat", "Etemp", "Eint", "Zet", "sdZet")
t.result <- system.time(result <- Pbugs(program="winbugs", data = data, inits = initials, parameters.to.save = param, model.file = NVA.NVA.t3.icar,
                                        n.chains = num.chains, n.iter = num.iter, n.burnin = num.burnin, bugs.directory = bugs.dir, DIC = FALSE))
resulta.nva.nva.icar.t3<- result 
t.resulta.nva.nva.icar.t3<- t.result
## rm
rm(list = c("cl", "initials", "param", "result", "t.result"))
rm("data")
...
initials<-function(){list(mu=rnorm(Ndiseases,0,0.1), sdstruct=runif(1,0,1), Spatial=matrix(rnorm(Nareas*Ndiseases), nrow=Ndiseases, ncol=Nareas), sdstructg=runif(1,0,1), Temporal=matrix(rnorm(Ndiseases*Nyears), nrow=Ndiseases, ncol=Nyears), sdZet=runif(Ndiseases,0,1),  Spatial.z=array(rnorm(Nareas*Ndiseases*Nyears),dim=c(Nyears,Ndiseases,Nareas)) ) }
param<-c("smr.prob", "eint.prob", "espat.prob", "etemp.prob", "SMR","lambda","mu", "Theta","M","sdstruct", "prec",  "Sigma.s", "Corre.s", "Gam","Mg", "sdstructg", "precg", "Sigma.t", "Corre.t", "Espat","Etemp", "Eint", "Zet", "sdZet")
t.result <- system.time(result <- Pbugs(program="winbugs", data = data, inits = initials, parameters.to.save = param, model.file = NVA.NVA.t4.icar,
                                        n.chains = num.chains, n.iter = num.iter, n.burnin = num.burnin, bugs.directory = bugs.dir, DIC = FALSE))
resulta.nva.nva.icar.t4<- result 
t.resulta.nva.nva.icar.t4<- t.result
## rm
rm(list = c("cl", "initials", "param", "result", "t.result"))
rm("data")
...
## save                               ##
########################################
resulta.winbugs.icar.re<- list(icar.ad.re=resulta.nva.nva.icar.ad,
                               icar.t1.re=resulta.nva.nva.icar.t1,
                               icar.t2.re=resulta.nva.nva.icar.t2,
                               icar.t3.re=resulta.nva.nva.icar.t3,
                               icar.t4.re=resulta.nva.nva.icar.t4)

t.resulta.winbugs.icar.re<- list(icar.ad.re=t.resulta.nva.nva.icar.ad,
                                 icar.t1.re=t.resulta.nva.nva.icar.t1,
                                 icar.t2.re=t.resulta.nva.nva.icar.t2,
                                 icar.t3.re=t.resulta.nva.nva.icar.t3,
                                 icar.t4.re=t.resulta.nva.nva.icar.t4)

save(resulta.winbugs.icar.re, t.resulta.winbugs.icar.re, file =paste0("resul/",gsub("\\.", "_", "resulta.winbugs.icar.re"),".RData"))
