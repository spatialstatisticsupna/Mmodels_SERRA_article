...
## load functions                                                             ##
################################################################################
source("functions_INLA/inla_rgeneric_Mmodel_model_pcar.R") # pcar
model.s <- inla.rgeneric.define(inla.rgeneric.Mmodel.model.pcar, debug = TRUE, k = k, W = W, alpha.min = alpha.min, alpha.max = alpha.max)
...
## Additive                           ##
########################################
pcar.ad.fe.fe <- inla(OBS ~ -1 + I1 + I2 +
...
## Type I                             ##
########################################
pcar.t1.fe.fe <- inla(OBS ~ -1 + I1 + I2 +
                      f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                      f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k))) +
...
## Type II                            ##
########################################
pcar.t2.fe.fe <- inla(OBS ~ -1 + I1 + I2 +
                      f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) +
                      f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## Type III
######################################################
pcar.t3.fe.fe<- inla(OBS ~ -1 + I1 + I2 +
                     f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k)) ) +
                     f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## Type IV                            ##
########################################
pcar.t4.fe.fe <- inla(OBS ~ -1 + I1 + I2 +
                      f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                      f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## save                               ##
########################################
resulta.inla.pcar.fe<- list(pcar.ad.fe=pcar.ad.fe.fe,
                            pcar.t1.fe=pcar.t1.fe.fe,
                            pcar.t2.fe=pcar.t1.fe.fe,
                            pcar.t3.fe=pcar.t3.fe.fe,
                            pcar.t4.fe=pcar.t4.fe.fe)
