...
## Additive                           ##
########################################
lcar.ad.re.re<- inla(OBS ~ -1 + I1 + I2 + 
...
## Type I                             ##
########################################
lcar.t1.re.re<- inla(OBS ~ -1 + I1 + I2 +
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                       f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k))) +
...
## Type II                            ##
########################################
lcar.t2.re.re <- inla(OBS ~ -1 + I1 + I2 +
                        f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) +
                        f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## Type III                           ##
########################################
lcar.t3.re.re<- inla(OBS ~ -1 + I1 + I2 +
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k)) ) + 
                       f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## Type IV                            ##
########################################
lcar.t4.re.re<- inla(OBS ~ -1 + I1 + I2 +
                       f(idx, model = model.s, constr=FALSE, extraconstr=list(A=A_constr.s, e=rep(0,k))) + 
                       f(idy, model = model.t, constr=FALSE, extraconstr=list(A=A_constr.t, e=rep(0,k)) ) +
...
## save                               ##
########################################
resulta.inla.lcar.re<- list(lcar.ad.re=lcar.ad.re.re,
                            lcar.t1.re=lcar.t1.re.re,
                            lcar.t2.re=lcar.t2.re.re,
                            lcar.t3.re=lcar.t3.re.re,
                            lcar.t4.re=lcar.t4.re.re)
save(resulta.inla.lcar.re, file=paste0(gsub("\\.", "_", "resulta.inla.lcar.re"), ".RData" ) )
