##################################################################################################
## Spatial: NVA (pcar); Temporal: NVA (rw1); Spatio-temporal: ad
## NVA.NVA.ad.pcar
##################################################################################################
NVA.NVA.ad.pcar <- function(){
  ...
  ##################
  for (j in 1:Ndiseases){
    Spatial[j,1:Nareas] ~ car.proper(ceros[],C.car.s[],adj[],num[],M.car.s[],1,gamma[j])  # structured
  ...
  gamma.inf<-min.bound(C.car.s[],adj[],num[],M.car.s[])
  gamma.sup<-max.bound(C.car.s[],adj[],num[],M.car.s[])
  ##################
  ## M-matrix (KxJ, K=2J)
  ##################
...
##################################################################################################
## Spatial: NVA (pcar); Temporal: NVA (rw1); Spatio-temporal: Type I
## NVA.NVA.t1.pcar
##################################################################################################
NVA.NVA.t1.pcar <- function(){
  for (j in 1:Ndiseases){
    for (i in 1:Nareas){
      for(l in 1:Nyears){
        ## Observed of the mean for each area, disease and time
...
##################################################################################################
## Spatial: NVA (pcar); Temporal: NVA (rw1); Spatio-temporal: Type II
## NVA.NVA.t2.pcar
##################################################################################################
NVA.NVA.t2.pcar <- function(){
  for (j in 1:Ndiseases){
    for (i in 1:Nareas){
      for(l in 1:Nyears){
        ## Observed of the mean for each area, disease and time
  ...
  ##################
  for (j in 1:Ndiseases){
    Spatial[j,1:Nareas] ~ car.proper(ceros[],C.car.s[],adj[],num[],M.car.s[],1,gamma[j])
    gamma[j]~dunif(gamma.inf,gamma.sup)
    for (i in 1:Nareas){
      tPhi[j,i] <-  c.Spatial[j,i]
      c.Spatial[j,i]<- Spatial[j,i] - mean(Spatial[j,])
...
##################################################################################################
## Spatial: NVA (pcar); Temporal: NVA (rw1); Spatio-temporal: Type III
## NVA.NVA.t3.pcar
##################################################################################################
NVA.NVA.t3.pcar <- function(){
  for (j in 1:Ndiseases){
    for (i in 1:Nareas){
      for(l in 1:Nyears){
        ## Observed of the mean for each area, disease and time
...
##################################################################################################
## Spatial: NVA (pcar); Temporal: NVA (rw1); Spatio-temporal: Type IV
## NVA.NVA.t4.pcar
##################################################################################################
NVA.NVA.t4.pcar <- function(){
  for (j in 1:Ndiseases){
    for (i in 1:Nareas){
      for(l in 1:Nyears){
        ## Observed of the mean for each area, disease and time
