...
## M-models (INLA)
########################################################################
'inla.rgeneric.Mmodel.model.pcar.n' <-
                   ...
                   "log.prior", "quit"), theta = NULL)
{
  ...
  interpret.theta = function()
  {
    ...
    M <- matrix(theta[-as.integer(c(1:k,(k+k*k+1)))], ncol = k)
    
    # The last parameter is sigma spatial
    sigma<- exp(-(1/2)*theta[as.integer(k+k*k+1)])
    return (list(alpha = alpha, M = M, sigma=sigma))
  }
  ########################################################################
  ...
  log.prior = function() {
    ...
    ## sigma spatial
    ###################################
    val = val + sum(log(0.005) - 0.5*theta[as.integer(k+k*k+1)])
    ###################################
    ## normal for mij
    ###################################
    val = val + sum(dnorm(theta[as.integer(c((k+1):(k*(k+1))))], mean=0, sd=param$sigma, log=TRUE))
    return (val)
  }
  ########################################################################
  ## initial
