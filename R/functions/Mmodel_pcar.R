...
#' In the \code{Mmodel_pcar_fe()} function the cells of the M-matrix are treated as fixed effects with a normal prior with mean 0 and large fixed variance (denoted as \code{FE} model).
#' In contrast, in the \code{Mmodel_pcar_re()} function the cells of the M-matrix are treated as independent normal random variables with mean 0 and standard deviation \eqn{\sigma} (denoted as \code{RE} model). In this case, a uniform prior distribution between 0 and a large number is considered for \eqn{\sigma}.
#'
...
#' where the columns of \eqn{\Phi} follow a proper CAR prior distribution (within-disease correlation) and \eqn{M} is a \eqn{k \times k} matrix which defines the correlation structure between-diseases.
...
#' \item \code{alpha.min}: lower limit defined for the uniform prior distribution of the spatial autocorrelation parameters
#' \item \code{alpha.max}: upper limit defined for the uniform prior distribution of the spatial autocorrelation parameters
#' }
#'
#' @references
...
#'
#' ## Define the M-model ##
#' Mmodel <- INLA::inla.rgeneric.define(Mmodel_pcar_fe, debug=FALSE, k=k, W=W,
#'                                      initial.values=initial.values, alpha.min=0, alpha.max=1)
#' Mmodel <- INLA::inla.rgeneric.define(Mmodel_pcar_re, debug=FALSE, k=k, W=W,
#'                                      initial.values=initial.values, alpha.min=0, alpha.max=1)
#'
