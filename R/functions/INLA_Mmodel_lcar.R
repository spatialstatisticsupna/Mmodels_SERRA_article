########################################################################
## Mmodels - LCAR - FE (BARTLETT DECOMPOSITION)
########################################################################
Mmodel_lcar <- function(cmd=c("graph","Q","mu","initial","log.norm.const","log.prior","quit"), theta=NULL){
  
  envir <- parent.env(environment())
  if(!exists("cache.done", envir=envir)){
    DW <- Diagonal(x=colSums(W))-W
  ...
  Q <- function(){
    ...
    BlockIW <- kronecker(diag(param$alpha),DW) + kronecker(diag(1-param$alpha),Diagonal(nrow(W)))
    Q <- (MI %*% BlockIW) %*% t(MI)
    return(Q)
  }
