...
## load functions                                                             ##
################################################################################
source("functions/WINBUGS_fe_fe_icar_ST.R")
...
param <- c("smr.prob","espat.prob", "etemp.prob", "SMR", "lambda", "mu", "Theta", "M",
           "Sigma.s", "Corre.s", "Gam", "Mg", "Sigma.t", "Corre.t", "Espat", "Etemp")

t.result <- system.time(result <- pbugs(program="winbugs", data = data, inits = initials,
                                        parameters.to.save = param, model.file = FE.FE.ad.icar,
                                        n.chains = num.chains, n.iter = num.iter,
                                        n.burnin = num.burnin, bugs.directory = bugs.dir,
                                        DIC = FALSE))

results.fe.fe.icar.ad <- result 
t.results.fe.fe.icar.ad <- t.result
...
param <- c("smr.prob", "eint.prob", "espat.prob", "etemp.prob", "SMR", "lambda",
           ...
           "Corre.t", "Espat", "Etemp", "Eint", "Zet", "sdZet")

t.result <- system.time(result <- pbugs(program="winbugs", data = data, inits = initials,
                                        parameters.to.save = param, model.file = FE.FE.t1.icar,
                                        n.chains = num.chains, n.iter = num.iter,
                                        n.burnin = num.burnin, bugs.directory = bugs.dir, DIC = FALSE))

results.fe.fe.icar.t1<- result 
t.results.fe.fe.icar.t1<- t.result
...
t.result <- system.time(result <- pbugs(program="winbugs", data = data, inits = initials,
                                        parameters.to.save = param, model.file = FE.FE.t2.icar,
                                        n.chains = num.chains, n.iter = num.iter,
                                        n.burnin = num.burnin, bugs.directory = bugs.dir,
                                        DIC = FALSE))

results.fe.fe.icar.t2 <- result 
t.results.fe.fe.icar.t2 <- t.result
...
t.result <- system.time(result <- pbugs(program="winbugs", data = data, inits = initials,
                                        parameters.to.save = param, model.file = FE.FE.t3.icar,
                                        n.chains = num.chains, n.iter = num.iter,
                                        n.burnin = num.burnin, bugs.directory = bugs.dir,
                                        DIC = FALSE))

results.fe.fe.icar.t3 <- result 
t.results.fe.fe.icar.t3 <- t.result
...
t.result <- system.time(result <- Pbugs(program="winbugs", data = data, inits = initials,
                                        parameters.to.save = param, model.file = FE.FE.t4.icar,
                                        n.chains = num.chains, n.iter = num.iter, 
                                        n.burnin = num.burnin, bugs.directory = bugs.dir,
                                        DIC = FALSE))

results.fe.fe.icar.t4 <- result 
t.results.fe.fe.icar.t4 <- t.result
...
## Save results                       ##
########################################
results.winbugs.icar.fe <- list(icar.ad.fe=results.fe.fe.icar.ad,
                                icar.t1.fe=results.fe.fe.icar.t1,
                                icar.t2.fe=results.fe.fe.icar.t2,
                                icar.t3.fe=results.fe.fe.icar.t3,
                                icar.t4.fe=results.fe.fe.icar.t4)

t.results.winbugs.icar.fe <- list(icar.ad.fe=t.results.fe.fe.icar.ad,
                                  icar.t1.fe=t.results.fe.fe.icar.t1,
                                  icar.t2.fe=t.results.fe.fe.icar.t2,
                                  icar.t3.fe=t.results.fe.fe.icar.t3,
                                  icar.t4.fe=t.results.fe.fe.icar.t4)

save(results.winbugs.icar.fe, t.results.winbugs.icar.fe, 
     file=paste0("results/",gsub("\\.", "_", "results.winbugs.icar.fe"),".RData"))
